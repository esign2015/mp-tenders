...
def find_search_area(page):
    ...
    if heading.count() == 0:
        return page
    # Walk up a few levels and pick the smallest ancestor containing a visible text input.
...
def do_search(page, tender_id):
    ...
    if not inputs:
        # Fallback: locate a visible input close to the Search heading by screen position.
    ...
    # Prefer an input whose metadata mentions search/id/tender.
    chosen = None
    for inp in inputs:
    ...
    for b in buttons:
        ...
        if re.fullmatch(r"go|search", txt, re.I) or re.search(r"\b(go|search)\b", meta, re.I):
            ...
            break
    if not go:
        # JSF fallback: submit the form containing the chosen input.
    ...
    # Find a result link that contains the requested ID or a likely tender-title link.
    ...
    for a in links:
        ...
        if tender_id.casefold() in (txt + " " + href).casefold():
            candidates.append(a)
    if not candidates:
        # Result title links are generally the first non-navigation links in the result table.
        for a in links:
            if not a.is_visible():
                continue
            txt = clean(a.inner_text())
            if txt and len(txt) > 8 and not re.search(r"^(home|search|more|next|previous|login)$", txt, re.I):
                if a.locator("xpath=ancestor::table[1]").count():
                    candidates.append(a)
    if not candidates:
        raise RuntimeError(f"Search returned no Tender Title result for {tender_id}; url={page.url}")

    title_link = candidates[0]
    ...
    if clean(detail.get("Tender ID")) != tender_id:
        # Some pages omit the ID in parser output; verify it exists in page text.
...
def main():
    ...
    STATUS.write_text(json.dumps({
        "tested": len(IDS), "success": len(rows), "failed": len(errors),
        "errors": errors, "updated_at": datetime.now(timezone.utc).isoformat(),
        "route": "Home -> Search with ID/Title/Reference no -> Tender ID -> GO -> Tender Title -> detail",
