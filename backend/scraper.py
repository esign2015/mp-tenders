...
def run_detail_validation(csv_file, organisations):
    ...
    def open_tender_by_click(page, tender_id):
        open_org(page)
        # The tender ID may be rendered in a nested/hidden span, so locating the
        # row by visible text is not reliable. Instead inspect the live detail
        # anchors generated by the portal and match the tender by ID/title/ref.
        links = page.locator("a")
        link_count = links.count()
        if link_count == 0:
            raise RuntimeError(f"No links found on live organisation list for {tender_id}")
        ...
        for i in range(link_count):
            ...
            try:
                ...
                if len(debug) < 12:
                    debug.append(f"{i}: {text[:160]} | {href[:220]}")
                hay = f"{text} {href} {onclick}".casefold()
                # Prefer a link whose visible text contains the tender title/ID/ref.
                if (
        ...
        if chosen is None:
            print("DETAIL LINK DEBUG:", " || ".join(debug))
            raise RuntimeError(f"Could not identify live detail link for {tender_id}")
        ...
        # A valid tender-detail page should identify the tender itself.
        has_id = tender_id.casefold() in body.casefold() or bool(TENDER_ID_RE.search(body))
        looks_like_nav_only = (
            "Tenders by Organisation" in body
            and "Tenders by Classification" in body
            and body.find("Tender Reference Number") < 0
            and body.find("EMD Amount") < 0
        )
        if looks_like_nav_only or not has_id:
            raise RuntimeError(
                f"Detail click did not open tender page: before={before} after={after}"
            )
        return soup, after
